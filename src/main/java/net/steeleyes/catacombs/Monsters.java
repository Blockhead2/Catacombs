...
public class Monsters {
  ...
  public void playerHeals(CatConfig cnf,Player healer, Player healee) {
    if(!healee.isDead()){
      ...
      if(!resp.wild) {
        int health = (int) healee.getHealth(); // TODO: verify cast
        if(health<20) {
  ...
  public void monsterHits(CatConfig cnf, EntityDamageEvent evt) {
    ...
    if(false) {
      ...
      final int before = (int) pp.getHealth(); // TODO: verify cast
      Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(plugin,new Runnable() {
        public void run() {
          int actual_dmg = (int) (before-pp.getHealth()); // TODO: verify cast
